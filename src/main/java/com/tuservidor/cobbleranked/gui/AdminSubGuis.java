...
class AdminEliteGui extends BaseGui {
    ...
    AdminEliteGui(ServerPlayerEntity player) {
        super(player, 4, "&d&lAlto Mando — Gestionar");
    }
    ...
    protected void build() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            LeagueMember found = e4.stream().filter(m -> m.getSlot() == pos).findFirst().orElse(null);
        ...
        setItem(31, GuiItem.of(GuiItem.BACK, "&7← Volver al Panel"), () -> new AdminMainGui(player).open());
    }
    ...
    protected void build() {
        ...
        if (!champs.isEmpty()) {
            ...
            setItem(13, GuiItem.of(GuiItem.CHAMPION_ICON,
                "&6&l👑 " + champ.getName(),
                "&7Victorias: &a" + champ.getWins() + " &7| Derrotas: &c" + champ.getLosses(),
            ...
            setItem(13, GuiItem.of(Items.NETHER_STAR,
                "&8Trono Vacante", "&7No hay Campeón registrado.", "", "&7Para añadir usa:", "&e/league add champ <jugador>"
            ));
        }
        setItem(22, GuiItem.of(GuiItem.BACK, "&7← Volver al Panel"), () -> new AdminMainGui(player).open());
    }
}
    ...
    protected void build() {
        ...
        for (int i = 0; i < Math.min(all.size(), slots.length); i++) {
            ...
            setItem(slots[i], GuiItem.of(
                ...
                m.getRole() == LeagueMember.Role.ELITE_FOUR  ? GuiItem.ELITE_ICON : GuiItem.CHAMPION_ICON,
        ...
        if (all.isEmpty()) setItem(22, GuiItem.of(Items.BARRIER, "&cSin miembros", "&7Nadie registrado en la liga aún."));
    ...
    protected void build() {
        ...
        if (!CobbleRanked.seasonActive) {
            setItem(11, GuiItem.of(Items.LIME_CONCRETE,
                ...
                CobbleRanked.server.execute(() ->
                    CobbleRanked.server.getPlayerManager().broadcast(
                        Text.literal(CobbleRanked.config.format(CobbleRanked.config.getMsgSeasonStart(), "%name%", name)), false));
            ...
            setItem(15, GuiItem.of(Items.RED_CONCRETE,
                ...
                CobbleRanked.config.setSeasonActive(false);
                CobbleRanked.config.saveAsync();
                
                CobbleRanked.server.execute(() ->
                    CobbleRanked.server.getPlayerManager().broadcast(
                        Text.literal(CobbleRanked.config.format(CobbleRanked.config.getMsgSeasonEnd(), "%name%", CobbleRanked.config.getSeasonName())), false));
                new AdminSeasonGui(player).open();
            });
        }
        setItem(22, GuiItem.of(GuiItem.BACK, "&7← Volver al Panel"), () -> new AdminMainGui(player).open());
...
class AdminQueueGui extends BaseGui {
    ...
    protected void build() {
        ...
        setItem(10, GuiItem.of(Items.GOLDEN_SWORD,
            "&6&lCola Ranked", "&7Jugadores: &e" + MatchmakingQueue.queueSize(QueueType.RANKED), "&7Distancia máx: &e50 bloques"
        ));
        setItem(14, GuiItem.of(Items.SHIELD,
            "&d&lCola Liga", "&7Jugadores: &e" + MatchmakingQueue.queueSize(QueueType.LEAGUE), "&7Distancia máx: &e50 bloques"
        ));
        setItem(22, GuiItem.of(Items.COMPASS,
            "&7El matchmaker corre cada &e5 segundos", "&7automáticamente en el servidor.", "", "&8Los jugadores deben estar en el", "&8mismo mundo a ≤50 bloques."
        ));
        setItem(49, GuiItem.of(GuiItem.BACK, "&7← Volver al Panel"), () -> new AdminMainGui(player).open());
    }
}
...
class AdminConfigGui extends BaseGui {
    ...
    protected void build() {
        ...
        setItem(28, GuiItem.of(Items.RED_DYE,   "&c-100 ELO Inicial"), () -> {
            CobbleRanked.config.setStartingElo(Math.max(0, CobbleRanked.config.getStartingElo() - 100));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });

        setItem(12, GuiItem.of(Items.EXPERIENCE_BOTTLE, "&bK-Factor (ELO por batalla)", "&7Valor actual: &e" + CobbleRanked.config.getKFactor(), "&8Más alto = más cambio de ELO", "", "&a+8 / &c-8"));
        ...
        setItem(30, GuiItem.of(Items.RED_DYE,   "&c-8 K-Factor"), () -> {
            CobbleRanked.config.setKFactor(Math.max(8, CobbleRanked.config.getKFactor() - 8));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });

        setItem(14, GuiItem.of(Items.CLOCK, "&eCooldown entre batallas", "&7Valor actual: &e" + CobbleRanked.config.getBattleCooldownSeconds() + " segundos", "", "&a+30s / &c-30s"));
        setItem(23, GuiItem.of(Items.LIME_DYE,  "&a+30s Cooldown"), () -> {
            CobbleRanked.config.setBattleCooldownSeconds(CobbleRanked.config.getBattleCooldownSeconds() + 30);
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });
        setItem(32, GuiItem.of(Items.RED_DYE,   "&c-30s Cooldown"), () -> {
            CobbleRanked.config.setBattleCooldownSeconds(Math.max(0, CobbleRanked.config.getBattleCooldownSeconds() - 30));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });

        setItem(16, GuiItem.of(Items.GOLDEN_SWORD, "&6Tamaño del Leaderboard", "&7Valor actual: &eTop " + CobbleRanked.config.getLeaderboardSize(), "", "&a+5 / &c-5"));
        setItem(25, GuiItem.of(Items.LIME_DYE,  "&a+5 Leaderboard"), () -> {
            CobbleRanked.config.setLeaderboardSize(Math.min(20, CobbleRanked.config.getLeaderboardSize() + 5));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });
        setItem(34, GuiItem.of(Items.RED_DYE,   "&c-5 Leaderboard"), () -> {
            CobbleRanked.config.setLeaderboardSize(Math.max(5, CobbleRanked.config.getLeaderboardSize() - 5));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });
        ...
        setItem(39, GuiItem.of(Items.RED_DYE, "&c-50 Umbral Plata"), () -> {
            CobbleRanked.config.setEloSilver(Math.max(0, CobbleRanked.config.getEloSilver() - 50));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });

        setItem(41, GuiItem.of(Items.GOLD_INGOT, "&6Umbral Oro: &e" + CobbleRanked.config.getEloGold(), "&8(ELO mínimo para Oro)", "&a+50 / &c-50"));
        ...
        setItem(43, GuiItem.of(Items.RED_DYE, "&c-50 Umbral Oro"), () -> {
            CobbleRanked.config.setEloGold(Math.max(0, CobbleRanked.config.getEloGold() - 50));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });
        ...
        setItem(47, GuiItem.of(Items.RED_DYE, "&c-50 Umbral Diamante"), () -> {
            CobbleRanked.config.setEloDiamond(Math.max(0, CobbleRanked.config.getEloDiamond() - 50));
            CobbleRanked.config.saveAsync(); new AdminConfigGui(player).open();
        });
